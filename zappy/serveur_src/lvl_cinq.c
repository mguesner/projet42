...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2014/06/18 14:09:26 by nguezell          #+#    #+#             */
/*   Updated: 2014/06/23 14:47:25 by mguesner         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		lvl_cinq(t_env *e, t_clients *player, t_case autel)
{
	...
	while (++i <= e->max_fd)
		if (e->clients[i].type == CLIENT_CLIENT
			...
			nb++;
	if (nb >= 4 && autel.linemate >= 1 && autel.deraumere >= 2
		&& autel.sibur >= 1 && autel.mendiane >= 3)
	{
		i = -1;
		player->lvl++;
		while (++i <= e->max_fd)
